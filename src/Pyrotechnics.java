...
public class Pyrotechnics implements Abilities {
	...
	public void attack(HeistCharacter character, HeistCharacter enemy) {
		...
		if (rand.nextInt(100) + 1 <= chanceToHit) {
			...
			System.out.println(character.getName() + " lights up fireworks at " + enemy.getName() + " burning them for "
					+ damage + " damage!");
			enemy.subtractHitPoints(damage);
		} else {
			System.out.println(
					character.getName() + " pyrotechnics shoot too erratically failing to hit " + enemy.getName());
			System.out.println();
		}
	}
